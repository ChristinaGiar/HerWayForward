...
const getCmsData = async (mock = false) => {
    ...
    for (const key of Object.keys(productsRawContent)) {
        if (products.length >= 3) { // return ONLY the first 3 products
            ...
            if (/card_[0-9]+/.test(key)) { // get ONLY the products with keys "card_1", "card_2", ... "card_9"
                const imageURL = imageDomain + productsRawContent[key].image; // e.g. 'http://localhost:8080' + '/content/luna/assets/images/catalog/Our Catalog.png'
